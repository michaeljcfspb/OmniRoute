...
| `DEFAULT_RATE_LIMIT_PER_DAY`            | _(empty)_               | `src/shared/utils/apiKeyPolicy.ts`       | Optional fallback per-day request budget for API keys without explicit rate-limit rules. Unset/empty means no implicit request cap, so UI-unrestricted keys stay unrestricted. Set to any positive integer N to enable N/day, 5N/week, 20N/month. Set to `0` to opt out explicitly. Malformed non-empty values use the legacy safety fallback (`1000/day`, `5000/week`, `20000/month`). |
